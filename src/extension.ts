...
async function resizeEditorGroups(centerWidth: number) {
    ...
    const layout = {
        orientation: 1,
        groups: [{
            groups: [{ size: sideWidth }, { size: centerWidth }, { size: sideWidth }],
            orientation: 0
        }]
    };
...
export function activate(context: vscode.ExtensionContext) {
    ...
    let layout: { left: vscode.TextDocument | null, center: vscode.TextDocument | null, right: vscode.TextDocument | null } = {
        left: null,
        center: null,
        right: null
    }
    ...
    disposable = vscode.window.onDidChangeActiveTextEditor(async (editor) => {
        ...
        if (editor.viewColumn === 1) {
            ...
            if (centerEditor && leftEditor) {
                await vscode.window.showTextDocument(leftEditor, { viewColumn: 2, preserveFocus: true });
        ...
        // we focused the right editor, swap center and right
            ...
            if (centerEditor && rightEditor) {
                await vscode.window.showTextDocument(rightEditor, { viewColumn: 2, preserveFocus: true });
